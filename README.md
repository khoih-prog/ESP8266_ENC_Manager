...
* [Why do we need this ESP8266_ENC_Manager library](#why-do-we-need-this-ESP8266_ENC_Manager-library)
  * [Features](#features)
...
* [Debug Terminal Output Samples](#debug-terminal-output-samples)
  * [1. ConfigOnDoubleReset_TZ using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60](#1-ConfigOnDoubleReset_TZ-using-LittleFS-on-ESP8266_NODEMCU_ESP12E-with-ESP8266_ENC28J60)
  * [2. ConfigOnSwichFS using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60](#2-ConfigOnSwichFS-using-LittleFS-on-ESP8266_NODEMCU_ESP12E-with-ESP8266_ENC28J60)
  * [3. ESP8266_FSWebServer_DRD using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60](#3-ESP8266_FSWebServer_DRD-using-LittleFS-on-ESP8266_NODEMCU_ESP12E-with-ESP8266_ENC28J60)
* [Debug](#debug)
...
### Why do we need this [ESP8266_ENC_Manager library](https://github.com/khoih-prog/ESP8266_ENC_Manager)
...
This [**ESP8266_ENC_Manager** library](https://github.com/khoih-prog/ESP8266_ENC_Manager) currently supports these following boards:

 1. **ESP8266 boards** using `LwIP ENC28J60 Ethernet`


<p align="center">
    <img src="https://github.com/khoih-prog/ESP8266_ENC_Manager/raw/main/Images/ENC28J60.png">
</p>
...
The best and easiest way is to use `Arduino Library Manager`. Search for `ESP8266_ENC_Manager`, then select / install the latest version. You can also use this link [![arduino-library-badge](https://www.ardu-badge.com/badge/ESP8266_ENC_manager.svg?)](https://www.ardu-badge.com/ESP8266_ENC_Manager) for more detailed instructions.

### Manual Install

1. Navigate to [ESP8266_ENC_Manager](https://github.com/khoih-prog/ESP8266_ENC_Manager) page.
2. Download the latest release `ESP8266_ENC_Manager-main.zip`.
3. Extract the zip file to `ESP8266_ENC_Manager-main` directory 
4. Copy the whole `ESP8266_ENC_Manager-main` folder to Arduino libraries' directory such as `~/Arduino/libraries/`.

### VS Code & PlatformIO:

1. Install [VS Code](https://code.visualstudio.com/)
2. Install [PlatformIO](https://platformio.org/platformio-ide)
3. Install [**ESP8266_ENC_Manager** library](https://registry.platformio.org/libraries/khoih-prog/ESP8266_ENC_Manager) by using [Library Manager](https://registry.platformio.org/libraries/khoih-prog/ESP8266_ENC_Manager/installation). Search for **ESP8266_ENC_Manager** in [Platform.io Author's Libraries](https://platformio.org/lib/search?query=author:%22Khoi%20Hoang%22)
...
```cpp
#include <ESP8266_ENC_Manager.hpp>               //https://github.com/khoih-prog/ESP8266_ENC_Manager
```

in many files. But be sure to use the following `#include <ESP8266_ENC_Manager.h>` **in just 1 `.h`, `.cpp` or `.ino` file**, which must **not be included in any other file**, to avoid `Multiple Definitions` Linker Error

```cpp
// To be included only in main(), .ino with setup() to avoid `Multiple Definitions` Linker Error
#include <ESP8266_ENC_Manager.h>          //https://github.com/khoih-prog/ESP8266_ENC_Manager
```

Have a look at the discussion in [Different behaviour using the src_cpp or src_h lib #80](https://github.com/khoih-prog/ESP8266_ENC_Manager/discussions/80)
...
// Must be placed before #include <ESP8266_ENC_Manager.h>
...
/******************************************
   // Defined in ESP8266_ENC_Manager.hpp
  typedef struct
  {
...
```cpp
//ESP8266_ENC_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask, dns1IP, dns2IP);
ESP8266_ENC_manager.setSTAStaticIPConfig(WM_STA_IPconfig);
```
...
```cpp
String tempTZ = ESP8266_ENC_manager.getTimezoneName();
```
...
```cpp
const char * TZ_Result = ESP8266_ENC_manager.getTZ(_timezoneName);
```
...
ESP8266_ENC_Manager ESP8266_ENC_manager();
```
...
```cpp
ESP8266_ENC_Manager ESP8266_ENC_manager("Personalized-HostName");
```
...
These illustrating steps is based on the example [ConfigOnSwitchFS](https://github.com/khoih-prog/ESP8266_ENC_Manager/tree/main/examples/ConfigOnSwitchFS)
...
The example [ConfigOnSwitchFS](https://github.com/khoih-prog/ESP8266_ENC_Manager/tree/main/examples/ConfigOnSwitchFS) will open the CP whenever a SW press is detected in `loop()`. So the code to add `dynamic variables` will be there, just after the CP `ESP8266_ENC_Manager` class initialization to create `ESP8266_ENC_Manager` object.
...
// is configuration portal requested?
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    ESP8266_ENC_Manager ESP8266_ENC_manager("ConfigOnSwitchFS");
...
Adding those `ESP8266_EMParameter` objects created in Step 2 using the function `addParameter()` of object `ESP8266_ENC_Manager`
...
ESP8266_ENC_manager.addParameter(&p_thingspeakApiKey);
ESP8266_ENC_manager.addParameter(&p_sensorDht22);
ESP8266_ENC_manager.addParameter(&p_pinSda);
ESP8266_ENC_manager.addParameter(&p_pinScl);
```
...
After users select `Save`, the CP `ESP8266_ENC_Manager` object will save the user input data into related `ESP8266_EMParameter` objects.
...
<p align="center">
    <img src="https://github.com/khoih-prog/ESP8266_ENC_Manager/raw/main/Images/Saved.png">
</p>
...
```cpp
ESP8266_ENC_manager.setSaveConfigCallback(saveConfigCallback);
```
...
```cpp
ESP8266_ENC_manager.setConfigPortalTimeout(120);
```
...
void loop()
{
  ...
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    ESP8266_ENC_Manager ESP8266_ENC_manager("ConfigOnSwitch");
    ...
    ESP8266_ENC_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask);
#endif
#endif

#if USING_CORS_FEATURE
    ESP8266_ENC_manager.setCORSHeader("Your Access-Control-Allow-Origin");
#endif
    ...
    else
    {
      ...
      ESP8266_ENC_manager.setConfigPortalTimeout(0);
    ...
    if (!ESP8266_ENC_manager.startConfigPortal())
    ...
    const char * TZ_Result = ESP8266_ENC_manager.getTZ(Ethconfig.TZ_Name);
    ...
    ESP8266_ENC_manager.getSTAStaticIPConfig(EthSTA_IPconfig);
...
Many applications need configuration parameters like `MQTT host and port`, [Blynk](http://www.blynk.cc) or [emoncms](http://emoncms.org) tokens, etc. While it is possible to use [`ESP8266_ENC_Manager`](https://github.com/khoih-prog/ESP8266_ENC_Manager) to collect additional parameters, it is better to read these parameters from a web service once [`ESP8266_ENC_Manager`](https://github.com/khoih-prog/ESP8266_ENC_Manager) has been used to connect to the Internet.

To capture other parameters with [`ESP8266_ENC_Manager`](https://github.com/khoih-prog/ESP8266_ENC_Manager) is a little bit more complicated than all the other features. This requires adding custom HTML to your form. 

If you want to do it with [`ESP8266_ENC_Manager`](https://github.com/khoih-prog/ESP8266_ENC_Manager) see the example [ConfigOnSwitchFS](examples/ConfigOnSwitchFS)
...
```cpp
ESP8266_ENC_manager.setSTAStaticIPConfig(IPAddress(192,168,2,186), IPAddress(192,168,2,1), IPAddress(255,255,255,0));
```
...
```cpp
ESP8266_ENC_manager.setCustomHeadElement("<style>html{filter: invert(100%); -webkit-filter: invert(100%);}</style>");
```
...
ESP8266_ENC_manager.addParameter(&custom_text);
```
...
https://github.com/khoih-prog/ESP8266_ENC_Manager/blob/d9d51a3df067fb25a605ae0ad037ca320de532cf/examples/ConfigOnSwitch/ConfigOnSwitch.ino#L27-L786
...
#### 1. ConfigOnDoubleReset_TZ using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60

##### 1.1 DRD => Config Portal

This is terminal debug output when running [ConfigOnDoubleReset](examples/ConfigOnDoubleReset) on  **ESP8266_ENC28J60**. `Config Portal` was requested by DRD to input and save Credentials, such as Static IP address.

```
Starting ConfigOnDoubleReset_TZ using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60 Ethernet
ESP8266_ENC_Manager v1.0.0
...
Saving config file...
Saving config file OK
Local Date/Time: Sun Dec 11 21:19:49 2022
Local Date/Time: Sun Dec 11 21:20:49 2022
Local Date/Time: Sun Dec 11 21:21:49 2022
Local Date/Time: Sun Dec 11 21:22:49 2022
```

---

#### 2. ConfigOnSwichFS using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60

This is terminal debug output when running [ConfigOnSwichFS](examples/ConfigOnSwichFS) on  **ESP8266_ENC28J60**. `Config Portal` was requested to input and save Credentials.


```
Starting ConfigOnSwichFS using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60 Ethernet
ESP8266_ENC_Manager v1.0.0
...
#### 3. ESP8266_FSWebServer_DRD using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60

This is terminal debug output when running [ESP8266_FSWebServer_DRD](examples/ESP8266_FSWebServer_DRD) on **ESP8266_ENC28J60**.
...
```cpp
Starting ESP_FSWebServer_DRD using LittleFS on ESP8266_NODEMCU_ESP12E with ESP8266_ENC28J60 Ethernet
ESP8266_ENC_Manager v1.0.0
ESP_DoubleResetDetector v1.3.2
Opening / directory
...
```cpp
ESP8266_ENC_manager.setDebugOutput(false);
```
...
Submit issues to: [ESP8266_ENC_Manager issues](https://github.com/khoih-prog/ESP8266_ENC_Manager/issues)
...
- The library is licensed under [MIT](https://github.com/khoih-prog/ESP8266_ENC_Manager/blob/main/LICENSE)
